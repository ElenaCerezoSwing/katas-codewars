...
const getTurkishNumber = (num) => {
  const unit = ['sıfır', 'bir', 'iki', 'üç', 'dört', 'beş', 'altı', 'yedi', 'sekiz', 'dokuz'];
  const tenth = ['on', 'yirmi', 'otuz', 'kırk', 'elli', 'altmış', 'yetmiş', 'seksen', 'doksan'];
  const firstDigit = parseInt(num.toString()[0]);
  const lastDigit = parseInt(num.toString()[1]);
  
  return num.toString().length === 1 ? unit[num] 
  : ( lastDigit === 0 ?  tenth[firstDigit -1] : tenth[firstDigit -1]  + ' ' + unit[lastDigit ]);
}